    ...
    public static CliOptions? Parse(string[] args)
    {
        ...
        try
        {
            return new CliOptions(
                ...
                MinVenues:        (int)ParseDecimal(map.GetValueOrDefault("min-venues"),
                                      new FlowSignalOptions().MinAgreeingVenues),
                StopAtr:          ParseDouble(map.GetValueOrDefault("stop-atr"),
                                      FlowGeometryDefaults.StopAtrMultiple),
                TargetRr:         ParseDouble(map.GetValueOrDefault("target-rr"),
                                      FlowGeometryDefaults.TargetRiskMultiple),
                MaxHoldHours:     ParseDouble(map.GetValueOrDefault("max-hold-hours"),
                                      FlowGeometryDefaults.MaxHoldHours),
                OosFraction:      Math.Clamp(ParseDouble(map.GetValueOrDefault("oos"), 0.4), 0.1, 0.9),
                SignalBars:       (int)ParseDecimal(map.GetValueOrDefault("signal-bars"),
