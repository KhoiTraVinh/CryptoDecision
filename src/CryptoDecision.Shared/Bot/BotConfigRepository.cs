    ...
    public async Task<BotOptions?> GetConfigAsync(CancellationToken ct = default)
    {
        const string sql = """
            SELECT enabled, paper_mode, symbol, exchange, active_strategies,
                   ...
                   daily_loss_limit_pct, eval_interval_seconds, grid_step_pct,
                   min_momentum_buy_ratio, min_buy_ratio_1h, use_trailing_stop,
                   trailing_stop_pct,
        ...
        return new BotOptions
        {
            ...
            ActiveStrategies         = ((string[])r.GetValue(4)).ToList(),
            CapitalUsd               = r.GetDecimal(5),
            MaxOpenTradesPerStrategy = r.GetInt32(6),
            PositionPctOfCapital     = r.GetDecimal(7),
            TakeProfitPct            = r.GetDecimal(8),
            StopLossPct              = r.GetDecimal(9),
            CooldownSeconds          = r.GetInt32(10),
            MaxHoldMinutes           = r.GetInt32(11),
            DailyLossLimitPct        = r.GetDecimal(12),
            EvalIntervalSeconds      = r.GetInt32(13),
            GridStepPct              = r.GetDecimal(14),
            MinMomentumBuyRatio      = r.GetDecimal(15),
            MinBuyRatio1h            = r.GetDecimal(16),
            UseTrailingStop          = r.GetBoolean(17),
            TrailingStopPct          = r.GetDecimal(18),
            UseBreakevenStop         = r.GetBoolean(19),
            BreakevenTriggerPct      = r.GetDecimal(20),
            UseDynamicTpSl           = r.GetBoolean(21),
            UseAiFilter              = r.GetBoolean(22),
            MinAiConfidence          = r.GetDecimal(23),
            UseAiSizing              = r.GetBoolean(24),
        };
    }
    ...
    public async Task StartBotAsync(BotOptions opts, CancellationToken ct = default)
    {
        const string sql = """
            ...
            SET enabled                      = TRUE,
                ...
                grid_step_pct                = @gridStep,
                min_momentum_buy_ratio       = @momRatio,
                min_buy_ratio_1h             = @buyRatio1h,
        ...
        cmd.Parameters.AddWithValue("gridStep",     NpgsqlDbType.Numeric, opts.GridStepPct);
        cmd.Parameters.AddWithValue("momRatio",     NpgsqlDbType.Numeric, opts.MinMomentumBuyRatio);
        cmd.Parameters.AddWithValue("buyRatio1h",   NpgsqlDbType.Numeric, opts.MinBuyRatio1h);
...
public sealed class BotConfigStatus
{
    ...
    public List<string> ActiveStrategies     { get; init; } = ["GRID", "MOMENTUM"];
    ...
    /// <summary>True if bot worker has sent a heartbeat within last 60 seconds.</summary>
    public bool IsWorkerAlive => LastHeartbeat.HasValue && (DateTime.UtcNow - LastHeartbeat.Value).TotalSeconds < 60;
}
